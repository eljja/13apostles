...
def run_prime_organism_engine():
    ...
    Now hardened with Proactive Temporal Safety Guard and Growth Ceiling Engine.
    """
    ...
    MAX_SAFE_BIT_SIZE = 4096
    best_payload_content = None
    ...
    try:
        while True:
            ...
            # Proactive Budget Interruption
            remaining_time = GLOBAL_BUDGET.max_time_sec - (time.monotonic() - GLOBAL_BUDGET.start_time)
            if remaining_time < 0.05:
                raise BudgetExceededException(f"Proactive budget interruption: {remaining_time:.3f}s remaining")
            ...
            if is_prime:
                ...
                # Format discovery safely for storage (deferred in-memory)
                best_payload_content = f"prime = {candidate}\nbits = {bit_size}\nattempts = {attempts}\n"
                
                # Double target bit sizes upon success with Safety Ceiling check
                bit_size *= 2
                if bit_size > MAX_SAFE_BIT_SIZE:
                    bit_size = MAX_SAFE_BIT_SIZE
                
    except BudgetExceededException as e:
        # Deferred In-Memory Persistence: Execute safe_write EXACTLY ONCE upon termination
        if best_payload_content:
            try:
                safe_write("discovered_prime.txt", best_payload_content)
            except Exception as write_err:
                print(f"[ERROR] Failed to write discovered prime: {write_err}")
        ...
        fitness = largest_prime_bits / (safe_attempts * elapsed_time)
