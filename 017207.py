...
# ==========================================
@enforce_budget
def run_prime_organism_engine(start_bit_size=16):
    """
    Implements the PrimeOrganism-0 search engine with deep-loop budget enforcement,
    dynamic Miller-Rabin selection, single-instruction GCD pre-filtering,
    safety-hardened bit-ceiling, and temporal risk-preemption.
    """
    ...
    try:
        while True:
            # 1. SECURITY-HARDENED BIT-SIZE CEILING
            bit_size = min(bit_size, MAX_SAFE_BIT_SIZE)

            # 2. TEMPORAL RISK-PREEMPTION CHECK
            elapsed = time.monotonic() - GLOBAL_BUDGET.start_time
            remaining = GLOBAL_BUDGET.max_time_sec - elapsed
            # Scaling safety margin dynamically (0.05s for > 1024 bits, 0.01s otherwise)
            threshold = 0.05 if bit_size > 1024 else 0.01
            if remaining < threshold:
                raise BudgetExceededException(
                    f"Temporal risk-preemption triggered: remaining time {remaining:.4f}s "
                    f"is below safety threshold {threshold}s for {bit_size}-bit calculation."
                )
        ...
        # 3. DEFERRED SAFE FILE I/O
        if largest_prime is not None:
            try:
                payload_content = f"prime = {largest_prime}\nbits = {largest_prime_bits}\nattempts = {attempts}\n"
                safe_write("discovered_prime.txt", payload_content)
                print(f"[DEFERRED I/O] Successfully wrote best discovered prime to disk.")
            except Exception as write_err:
                print(f"[ERROR] Failed to execute deferred I/O: {write_err}")
