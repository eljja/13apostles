...
# Hard security boundary for atomic exponentiation to prevent thread lockups
...
def run_prime_organism_engine():
    ...
    try:
        while True:
            ...
            # Witness Generator Sanitization: deterministic check for <= 3
            if candidate <= 3:
            ...
            if is_prime:
                ...
                # Non-Catastrophic I/O Graceful Fallback
                try:
                    safe_write("discovered_prime.txt", payload_content)
                except (OSError, PermissionError, IOError):
                    print("[WARNING] File write failed, degrading to safe in-memory state tracking...")
                
                # Double target bit sizes upon success, clamped by the security boundary ceiling guard
                bit_size = min(bit_size * 2, MAX_SAFE_BIT_SIZE)
