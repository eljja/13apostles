...
# ==========================================
# PRECOMPUTED GCD FILTER INITIALIZATION
# ==========================================
def _init_primes_and_product():
    primes = []
    sieve = [True] * 2000
    for p in range(2, 2000):
        if sieve[p]:
            primes.append(p)
            for i in range(p*p, 2000, p):
                sieve[i] = False
    ...
    return set(primes), prod

PRIMES_UNDER_2000_SET, PRIME_PRODUCT = _init_primes_and_product()

# ==========================================
# MATHEMATICAL ENGINE: PrimeOrganism-0 (W-CGCD Optimized)
# ==========================================
def is_probable_prime(n, k=5):
    """
    Miller-Rabin primality test with single-call C-GCD pre-sieve.
    """
    if n <= 1: return False
    if n < 2000:
        return n in PRIMES_UNDER_2000_SET

    if n % 2 == 0 or n % 3 == 0 or n % 5 == 0:
        return False

    # Single-Call C-Optimized GCD Pre-Filtering
...
def prime_organism_engine():
    ...
    Uses Modulo-30 Wheel-Parameterized Candidate Generation.
    """
    best_bits = 0
    total_attempts = 0
    current_bits = 16
    ...
    wheel_remainders = (1, 7, 11, 13, 17, 19, 23, 29)

    try:
        while True:
            GLOBAL_BUDGET.enforce()
            
            # Generate candidate of exact target bit-length using Modulo-30 Wheel
            m_min = (2**(current_bits - 1) + 29) // 30
            m_max = (2**current_bits - 1 - 29) // 30
            candidate = 30 * random.randint(m_min, m_max) + random.choice(wheel_remainders)
