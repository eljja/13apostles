...
def run_prime_organism_engine():
    ...
    # Transient In-Memory Registry for tracking search metrics without intermediate disk I/O
    state_registry = {
        "largest_prime": None,
    ...
    try:
        while True:
            ...
            # Dynamic selection of Miller-Rabin rounds (k) based on target bit size
            ...
            state_registry["attempts"] = attempts
            ...
            if is_prime:
                state_registry["largest_prime"] = candidate
                state_registry["largest_prime_bits"] = bit_size
        ...
        safe_attempts = max(state_registry["attempts"], 1)
        fitness = state_registry["largest_prime_bits"] / (safe_attempts * elapsed_time)
        
        print(f"\n[SUCCESS] Enforcer interrupted PrimeOrganism-0: {e}")
        print(f"Graceful termination completed.")
        print(f"Final Performance Metrics:")
        print(f"  - Max Bit Size Discovered: {state_registry['largest_prime_bits']}")
        print(f"  - Total Candidates Evaluated: {state_registry['attempts']}")
        print(f"  - Safe Elapsed Time: {elapsed_time:.6f}s")
        print(f"  - Final Fitness Score: {fitness:.4f}")
        
        # Standardized JSON Storage Schema written EXACTLY ONCE upon termination
        payload = {
            "prime": state_registry["largest_prime"],
            "prime_str": str(state_registry["largest_prime"]) if state_registry["largest_prime"] is not None else None,
            "bits": state_registry["largest_prime_bits"],
            "attempts": state_registry["attempts"],
            "elapsed_time": elapsed_time,
            "fitness": fitness
        }
        
        try:
            safe_write("discovered_prime.json", json.dumps(payload, indent=2))
            print(f"[SUCCESS] Serialized discovered state registry to discovered_prime.json")
        except Exception as write_err:
            print(f"[ERROR] Failed to serialize transient state registry: {write_err}")
...
def evolve():
    ...
    try:
        ...
        for temp_file in ["discovered_prime.json", "synthetic_dummy_output.py"]:
            try:
