...
# ==========================================
# STATIC IMMUTABLE PRIMORIAL INITIALIZATION (CPG-DMC)
# ==========================================
def _generate_odd_primes():
    primes = []
    for num in range(3, 542, 2):
        is_prime = True
...
ODD_PRIMES = _generate_odd_primes()
PRIMORIAL_ODD = 1
for p in ODD_PRIMES:
    PRIMORIAL_ODD *= p
...
def is_probable_prime(n):
    """
    Miller-Rabin primality test with dynamic round calibration, C-native primorial GCD pre-filtering,
    and embedded temporal checks.
    """
    # Deterministic Range Guard
    if n <= 541:
        return n == 2 or n in ODD_PRIMES
    
    # Pre-filtering: Even numbers and C-native GCD check
    if n % 2 == 0:
        return False
    if math.gcd(n, PRIMORIAL_ODD) != 1:
        return False

    # Dynamic Round Calibration
