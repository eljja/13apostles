...
def synthetic_infinite_loop():
    ...
    while True:
        ...
        loop_counter += 1
        if loop_counter % 5 == 0:
            print(f"Iteration {loop_counter} | Tokens: {GLOBAL_BUDGET.tokens_used} | Time: {time.monotonic() - GLOBAL_BUDGET.start_time:.3f}s")
    ...
    try:
        ...
        if os.path.exists("synthetic_dummy_output.py"):
            try:
                ...
                print(f"[WARNING] Safe cleanup failed to remove synthetic file: {cleanup_err}")
