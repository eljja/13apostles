...
# ==========================================
# DECOUPLED STATE REGISTRY & PERSISTENCE
# ==========================================
class SearchState:
    """
    A lightweight, cohesive state registry acting as the single source of truth
    for the search telemetry in transient memory.
    """
    def __init__(self):
        self.largest_prime = None
        self.largest_prime_bits = 0
        self.attempts = 0

def persist_state(state: SearchState):
    """
    Executes deferred safe persistence of the search state exactly once.
    """
    if state.largest_prime is not None:
        payload_content = f"prime = {state.largest_prime}\nbits = {state.largest_prime_bits}\nattempts = {state.attempts}\n"
        safe_write("discovered_prime.txt", payload_content)
        print(f"[PERSIST] Decoupled state persisted successfully: {state.largest_prime_bits}-bit prime.")
    else:
        print("[PERSIST] No verified prime discovered to persist.")

# ==========================================
# PRE-COMPUTED GCD SIEVE INVARIANT SETUP
# ==========================================
def _generate_primes_and_product(limit=1000):
    """
    Sieve of Eratosthenes to find all primes up to 1000, 
    and pre-compute their scalar product for C-level fast GCD filtering.
    """
    sieve = [True] * (limit + 1)
    ...
    for p in primes:
        product *= p
    return set(primes), product

SMALL_PRIMES_SET, PRIME_PRODUCT = _generate_primes_and_product(1000)
...
def run_prime_organism_engine():
    ...
    dynamic Miller-Rabin selection, and single-instruction GCD pre-filtering.
    Now optimized with a decoupled transient state registry and deferred persistence.
    """
    ...
    try:
        while True:
            ...
            state.attempts = max(0, state.attempts + 1) # Guard against counter overflows
            
            # Use pre-computed GCD sieve for instant rejection of composites
            if candidate < 1000:
                is_prime = candidate in SMALL_PRIMES_SET
            else:
                if math.gcd(candidate, PRIME_PRODUCT) > 1:
                    is_prime = False
                else:
                    is_prime = True
            ...
            if is_prime:
                state.largest_prime = candidate
                state.largest_prime_bits = bit_size
                print(f"[FOUND] {bit_size}-bit prime found: {candidate} (attempts: {state.attempts})")
                
                # Double target bit sizes upon success
                bit_size *= 2
                
    except BudgetExceededException as e:
        # DEFERRED PERSISTENCE: Write transient state to disk exactly once during shutdown
        persist_state(state)
        
        # Calculate metrics using zero-division guards and clean conversions
        elapsed_time = max(time.monotonic() - start_run, 0.000001)
        safe_attempts = max(state.attempts, 1)
        fitness = state.largest_prime_bits / (safe_attempts * elapsed_time)
        
        print(f"\n[SUCCESS] Enforcer interrupted PrimeOrganism-0: {e}")
        print(f"Graceful termination completed.")
        print(f"Final Performance Metrics:")
        print(f"  - Max Bit Size Discovered: {state.largest_prime_bits}")
        print(f"  - Total Candidates Evaluated: {state.attempts}")
...
def evolve():
    ...
    try:
        ...
        # Exception-Resilient Cleanup of workspace contents (Preserving discovered_prime.txt for verification)
        print("Executing cleanup pipeline...")
        for temp_file in ["synthetic_dummy_output.py"]:
            try:
        ...
        # Attempt to prune empty safe_workspace directory (will not prune if discovered_prime.txt exists)
        try:
