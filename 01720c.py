    ...
    try:
        while True:
            ...
            if is_prime:
                ...
                attempts_at_discovery = attempts
                print(f"[FOUND] {bit_size}-bit prime found: {candidate} (attempts: {attempts})")
                
                # Smooth Adaptive Bit-Scaling Transition
                if bit_size < 256:
                    bit_size *= 2
                else:
                    bit_size = int(round((bit_size * 1.5) / 64.0) * 64)
                
    except BudgetExceededException as e:
        # Deferred State-Preserving I/O: Execute exact-once single safe_write upon budget exhaustion
        if largest_prime is not None:
            try:
                payload_content = f"prime = {largest_prime}\nbits = {largest_prime_bits}\nattempts = {attempts_at_discovery}\n"
                safe_write("discovered_prime.txt", payload_content)
            except Exception as write_err:
                print(f"Warning: Failed to save best prime to file upon budget exhaustion: {write_err}")
